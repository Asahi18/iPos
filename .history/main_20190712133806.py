...
def save_order(tableno,foods,drinks):
	...
	with open('kitchen_f.csv','a') as f:
		writer=csv.writer(f)
		kitchen_food=[datetime.datetime(),tableno,]+fc
		writer.writerow(kitchen_food)

	with open('kitchen_d.csv','a') as f:
		writer=csv.writer(f)
		kitchen_drink=[datetime.datetime(),tableno,]+dc
...
def kitchen_food():
	...
	return render_template("kitchen_food.html",title='kitchen_food',df=df_f)
...
def kitchen_drink():
	...
	dnt=['time','table']+dn
	df_d=pd.read_csv('kitchen_d.csv',names=dnt)
	return render_template("kitchen_drink.html",title='kitchen_drink',df=df_d)
