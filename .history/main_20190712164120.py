...
def kitchen_food():
	...
	for row in reader:
		ft.append(row)
	f.close()
	return render_template("kitchen_food.html",title='kitchen_food',fnt=fnt,ft=ft,i=1)
...
def kitchen_drink():
	...
	for row in reader:
		dt.append(row)
	f.close()
	return render_template("kitchen_drink.html",title='kitchen_drink',dnt=dnt,dt=dt,i=1)

@app.route("/history")
def history():
	return render_template("history.html",title='history')

@app.route("/serve_f",methods=['POST'])
def serve_f():
	ft=[]
	f=open('kitchen_f.csv','r') 
	reader=csv.reader(f)
	for row in reader:
		ft.append(row)
	f.close()
	idx=int(request.form['servedno_f'])-1
	ft.remove(request.form[idx])
	with open('kitchen_f.csv','w') as f:
		writer=csv.writer(f)
		writer.writerows(ft)
	return render_template("served.html")

@app.route("/serve_d",methods=['POST'])
def serve_d():
	dt=[]
	f=open('kitchen_d.csv','r') 
	reader=csv.reader(f)
	for row in reader:
		dt.append(row)
	f.close()
	idx=int(request.form['servedno_d'])-1
	dt.remove(request.form[idx])
	return render_template("served.html")
	return "served!"
