...
def save_order(tableno,foods,drinks):
	...
	with open('kitchen_f.csv','a') as f:
		writer=csv.writer(f)
		kitchen_food=[datetime.time(),tableno,]+fc
		writer.writerow(kitchen_food)

	with open('kitchen_d.csv','a') as f:
		writer=csv.writer(f)
		kitchen_drink=[datetime.time(),tableno,]+dc
...
def kitchen_food():
	...
	for food in foods:
		fn.append(food.name)
	fnt=['time','table']+fn
	df_f=read_csv('kitchen_f.csv',name=fnt)
	return render_template("kitchen_food.html",title='kitchen_food')
...
def kitchen_drink():
	...
	dnt=['time','table']+dn
	df_d=read_csv('kitchen_d.csv',name=dnt)
	return render_template("kitchen_drink.html",title='kitchen_drink')
